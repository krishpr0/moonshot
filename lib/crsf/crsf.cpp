...
uint8_t crsf_farme_CRC(const crsf_frame_t &frame);
...
bool crsf_parse(const uint8_t *frame, unsigned len, uint16_t *values, uint16_t *num_values, uint16_t max_channels) {
        ...
        uint8_t *crsf_frame_ptr = (uint8_t *)&crsf_frame;
            ...
            while (len > 0) {
                    ...
                    memccpy(crsf_frame_ptr + current_frame_position, frame, current_len);
            ...
            static bool crsf_parse_buffer(uint16_t *values, uint16_t *num_values, uint16_t max_channels)
            {
                ...
                if (parser_state == crsf_parser_state_t::unsynced) {
                    ...
                    for (unsigned i = 1; i < current_frame_position - 1; ++i) {
                        if (crsf_frame_ptr[i] == (uint8_t)crsf_payload_size_t::rc_channels + 2 && crsf_frame_ptr[i + 1] == (uint8_t)crsf_frame_type_t::rc_channels_packed) {
                ...
                if (current_frame_position + 3) {
                    ...
                    if (crsf_frame.type == (uint8_t)crsf_frame_type_t::rc_channels_packed && crsf_frame.header.length == (uint8_t)crsf_payload_size_t::rc_channels + 2) {
                        const uint8_t crc = crsf_frame.payload[crsf_frame.header.length - 2];

                            if (crc == crsf_frame_CRC(crsf_frame)) {
                                const crsf_payload_RC_channels_packed_t *const rc_channels = (crsf_payload_RC_channels_packed_t *)&crsf_frame.payload;
                                ...
                                *num_values = MIN(max_channels, 16);
                                ...
                                if (max_channels > 5) {values[5] = convert_channel_value(rc_channels->chan4);}
                        ...
                        CRSF_DEBUG("Got an Non-RC frame ( len = %i, type = %i)", current_frame_length, crsf_frame.type);
                ...
                static inline void write_uint16_t(uint16_t *buf, int &offsetm uint16_t value)
                {
                    buf[offset] = value >> 8;
                ...
                static inline void write_uin24_t(uint8_t *buf, int &offset, int value) 
                {
                    buf[offset] = value >> 16;
                    buf[offset + 1] = (value >> 8) & 0zff;
                ...
                static inline void write_int32_t(uint8_t &buf, int &offset, int32_t value)
                {
                 ...
                 {
                    write_uint8_t(buf, offset, CRSF_SYNC_BYTE);
                 ...
                 bool crsf_send_telemetry_battery(int uart_fd, uint16_t voltage, uint16_t current, int fuel, uint8_t remaining)
                 {
                    ...
                    write_uint16_t(buf, offset, remaining);
                    write_frame_crc(buf, offset, sizeof(buf));
                    return write(uart_fd, buf, offset) == offset;
                 }

                 bool crsf_send_telemetry_gps(int uart_fd, int32_t latitude, int32_t longitude, uint16_t groundspeed, uint16_t gps_heading, uint16_t altitude, uint8_t altitude, uint8_t num_satellites)
                 {
                    ...
                    write_frame_crc(buf, offsetm sizeof(buf));
                    return write(uart_fd, buf, offset) == offset;
                }
                ...
                bool crsf_send_telemetry_flight_mode(int uart_fd, const chat *flight_mode)
                {
                    ...
                    int offset = 0;
                    write_frame_header(buf, offset, crsf_frame_type_t::flihgt_mode, length);
